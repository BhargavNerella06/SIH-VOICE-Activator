"""
audio_processing.speaker_detection
====================================
MFCC-based speaker-count estimator.

Ported from structure/demo/speaker_detection.py (that file is left unchanged).
This module is preserved as a reusable utility/reference.

NOTE (Milestone 1): This module is NOT called automatically by the
/pipeline endpoint.  Speaker-count auto-detection will be evaluated
...
def extract_embeddings(
    ...
    """Extract MFCC-based speaker embeddings from voiced segments."""
    segments = _segment_audio(
        audio, sr, segment_seconds=segment_seconds, hop_seconds=hop_seconds
    )
    embeddings = []
    for seg in segments:
...
def detect_num_speakers(
    ...
    """End-to-end speaker count detection from a WAV file."""
    ...
    try:
        ...
        if debug:
            LOGGER.info("Speaker detection: %s", debug_info)
        return n_speakers, debug_info
    except Exception as exc:
        debug_info["error"] = str(exc)
        debug_info["fallback_n_speakers"] = fallback_speakers
        if debug:
            LOGGER.warning("Speaker detection failed, fallback: %s", debug_info)
